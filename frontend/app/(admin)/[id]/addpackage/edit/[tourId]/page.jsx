...
export default function EditTourPage() {
  ...
  const { createTour, updateTour, exists, checkTourId, isChecking, gallery, loading, fetchGallery, fetchTour, tour } =
    useTourStore()
  ...
  const toggleExpand = (index) => {
    setItineraryList((prev) => prev.map((item, i) => (i === index ? { ...item, expanded: !item.expanded } : item)))
  }

  const handleItineraryChange = (index, field, value) => {
    setItineraryList((prev) => prev.map((item, i) => (i === index ? { ...item, [field]: value } : item)))
  }
  ...
  const filteredLocations = locations.filter((loc) =>
    activeField && formState[activeField] ? loc.name.toLowerCase().includes(formState[activeField].toLowerCase()) : [],
  )
  ...
  const filteredCategories = categories.filter((cat) =>
    formState.category ? cat.name.toLowerCase().includes(formState.category.toLowerCase()) : [],
  )
  ...
  useEffect(() => {
    if (tour) {
      setFormState({
        ...
        startDate: tour.startDate ? new Date(tour.startDate).toISOString().split("T")[0] : getToday(),
        endDate: tour.endDate ? new Date(tour.endDate).toISOString().split("T")[0] : getToday(),
        status: tour.status || "",
    ...
    if (itineraries?.length) {
      setItineraryList(
        itineraries.map((item) => ({
          ...
          date: item.date ? new Date(item.date).toISOString().split("T")[0] : getToday(),
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <Card>
          <CardContent className="pt-6">
            ...
            <p className="text-sm text-gray-500 mb-4">Manage your tour images (maximum 10 images)</p>
            ...
            <div className="flex flex-wrap gap-3 mt-4">
              ...
              {/* Existing images */}
              {gallery.map((img, idx) => (
                <div key={`existing-${idx}`} className="relative h-24 w-24 rounded-md overflow-hidden group">
                  <img src={img.url || img} alt={`preview-${idx}`} className="h-full w-full object-cover" />
                  <button
                    type="button"
              ...
              {newImages.map((img, idx) => (
                <div key={`new-${idx}`} className="relative h-24 w-24 rounded-md overflow-hidden group">
                  <img
                    src={URL.createObjectURL(img) || "/placeholder.svg"}
                    alt={`preview-new-${idx}`}
                    className="h-full w-full object-cover"
                  />
                  <Badge className="absolute top-1 left-1 bg-teal-500 text-[10px]">New</Badge>
                  <button
                    type="button"
            ...
            <p className="text-sm text-gray-500 mt-3">{gallery.length + newImages.length} of 10 images</p>
          </CardContent>
        </Card>
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  {!isChecking && exists === true && formState.tour_id !== tour?.tour_id && (
                ...
                <div key={field} className="relative">
                  <label htmlFor={field} className="block text-sm font-medium text-gray-700 mb-1">
                    {field
                      ...
                      .join(" ")}
                    {field === "start_location" && <span className="text-red-500"> *</span>}
                  </label>
                  <div className="relative">
                    <Input
                      id={field}
                    ...
                    {formState[field] && (
                      <button
                        type="button"
                        onClick={() => setFormState((prev) => ({ ...prev, [field]: "" }))}
          ...
          <CardContent className="pt-6">
            <div className="mb-4 flex items-center justify-between">
              ...
              <Badge variant="outline" className="text-teal-600 border-teal-200 bg-teal-50">
                {itineraryList.length} {itineraryList.length === 1 ? "day" : "days"}
              </Badge>
            </div>

            <p className="text-sm text-gray-500 mb-6">Manage the daily activities and schedules for your tour</p>
